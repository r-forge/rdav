...
simultaneously
- If you want to work only in a subdirectory, e.g. `/sub/dir`, you can append it to the
*WebDAV* URL
`https://cloud.example.com/remote.php/dav/files/myname/sub/dir`.
Then you don't have to specify the full path when working with files in this
directory with `wd_*` functions. **Notice**: for managing shares via `ocs_*` functions you still need
to give the full path to files or folders.
